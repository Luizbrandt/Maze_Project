...
class Player:
    ...
    def onestep(self):
        if self.canwalk:
            validDirections = list(
                filter(lambda d: self._isvaliddir(d), self.dirs))
            if len(validDirections) > 0:
                self.path.append(self._getnewpos(
                    random.choice(validDirections)))
    ...
    def evaluate(self):
        self.walk()
        lp = self.path[-1]
        distance = len(pathfinding(
            lp, self.end, self.maze.getMaze())) if not self.win else 0
        # distance = manhattan(lp[1],lp[0], self.end[1], self.end[0])
        self.fitness = mapFromTo(
            distance*distance, self.maxdistance*self.maxdistance, 0, 0, 1)
    ...
    def mutate(self):
        self.path = self.path[:int(
            mapFromTo(random.random(), 0, 1, 1, len(self.path)))]
